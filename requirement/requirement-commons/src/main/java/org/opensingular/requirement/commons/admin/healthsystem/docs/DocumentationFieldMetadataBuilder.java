...
import org.opensingular.requirement.commons.admin.healthsystem.DocumentationMetadataUtil;
...
class DocumentationFieldMetadataBuilder implements Loggable {
    ...
    public DocumentationFieldMetadataBuilder(SType<?> rootType, SType<?> type) {
        ...
        if (isFormInputField()) {
            docFieldMetadata = new DocFieldMetadata(
                    ...
                    DocumentationMetadataUtil.resolveDependsOn(rootType, type),
                    initMask(type),
                    initMaxSize(type),
                    initMinSize(type),
    ...
    private String initFieldSubtitle(SType<?> type) {
        return StringUtils.defaultString(DocumentationMetadataUtil.getAttribute(type, SPackageBasic.ATR_SUBTITLE).orElse(null), null);
    }
    ...
    private String initMask(SType<?> type) {
        return StringUtils.defaultString(DocumentationMetadataUtil.getAttribute(type, SPackageBasic.ATR_BASIC_MASK).map(InputMaskBehavior.Masks::valueOf).map(InputMaskBehavior.Masks::getMask).orElse(null), null);
    }
